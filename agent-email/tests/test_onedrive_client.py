...
def test_upload_attachments_raises_descriptive_error_on_http_failure(mock_put):
    mock_put.return_value.raise_for_status.side_effect = requests.HTTPError("401 Unauthorized")
    mock_put.return_value.text = "Invalid token"
    message = _message(
        attachments=[Attachment(filename="test.pdf", content_type="application/pdf", content=b"data")]
    )

    with pytest.raises(RuntimeError) as exc_info:
        upload_attachments("bad-token", message, "Rechnungen/Zahlungen")

    error_message = str(exc_info.value)
    assert "test.pdf" in error_message
    assert "Rechnungen/Zahlungen" in error_message
...
def test_upload_attachments_raises_descriptive_error_on_invalid_response(mock_put):
    mock_put.return_value.json.side_effect = ValueError("No JSON object could be decoded")
    mock_put.return_value.text = "Invalid JSON"
    message = _message(
